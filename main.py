...
class T5FineTuner(pl.LightningModule):
    ...
    def __init__(self, hparams, tfm_model, tokenizer):
        ...
        self.hparams = hparams   # 本身的
        # self.save_hyperparameters(hparams)  # 我自己修改的，版本不一样会报错
    ...
    def optimizer_step(self, epoch, batch_idx, optimizer, optimizer_idx, second_order_closure=None):
        if self.trainer.use_tpu:
            # xm.optimizer_step(optimizer)  # pycharm不能跑，但是colab能跑
            self.optimizer_step(optimizer)  # 我自己修改的
        else:
            optimizer.step()
        optimizer.zero_grad()
